...
class YOLODataset(BaseDataset):
    ...
    def __init__(self, *args, data=None, task="detect", **kwargs):
        ...
        self.data = data
        assert not (
            self.use_segments and self.use_keypoints), "Can not use both segments and keypoints."
    ...
    def get_labels(self):
        ...
        try:
            cache, exists = load_dataset_cache_file(
        ...
        if exists and LOCAL_RANK in {-1, 0}:
            ...
            TQDM(None, desc=self.prefix + d, total=n,
        ...
        lengths = ((len(lb["cls"]), len(lb["bboxes"]),
                   len(lb["segments"])) for lb in labels)
    ...
    def update_labels_info(self, label):
        ...
        if len(segments) > 0:
            ...
            segments = np.stack(resample_segments(
                segments, n=segment_resamples), axis=0)
        else:
...
class YOLOMultiModalDataset(YOLODataset):
    ...
    def build_transforms(self, hyp=None):
        ...
        if self.augment:
            ...
            transforms.insert(-1, RandomLoadText(
                max_samples=min(self.data["nc"], 80), padding=True))
        return transforms
    ...
    def get_labels(self):
        ...
        for img_id, anns in TQDM(img_to_anns.items(), desc=f"Reading annotations {self.json_file}"):
            ...
            for ann in anns:
                ...
                cat_name = " ".join([img["caption"][t[0]: t[1]]
                ...
                if box not in bboxes:
                    bboxes.append(box)
            lb = np.array(bboxes, dtype=np.float32) if len(
                bboxes) else np.zeros((0, 5), dtype=np.float32)
            labels.append(
                {
                    "im_file": im_file,
                    "shape": (h, w),
...
class ClassificationDataset:
    ...
    def __init__(self, root, args, augment=False, prefix=""):
        ...
        if augment and args.fraction < 1.0:  # reduce training fraction
            self.samples = self.samples[: round(
                len(self.samples) * args.fraction)]
        self.prefix = colorstr(f"{prefix}: ") if prefix else ""
        self.cache_ram = args.cache is True or str(
            args.cache).lower() == "ram"  # cache images into RAM
        if self.cache_ram:
            LOGGER.warning(
    ...
    def __getitem__(self, i):
        ...
        f, j, fn, im = self.samples[
            i]  # filename, index, filename.with_suffix('.npy'), image
        if self.cache_ram:
    ...
    def verify_images(self):
        ...
        try:
            ...
            assert cache["version"] == DATASET_CACHE_VERSION
            assert cache["hash"] == get_hash(
                [x[0] for x in self.samples])  # identical hash
            # found, missing, empty, corrupt, total
            ...
            with ThreadPool(NUM_THREADS) as pool:
                results = pool.imap(func=verify_image, iterable=zip(
                    self.samples, repeat(self.prefix)))
